...
async function carregarTurmas() {
    ...
    try {
        ...
        turmas.forEach((turma, index) => {
            grid.appendChild(criarCardTurma(mapearTurma(turma, index)));
        });
...
function mapearTurma(turma, index) {
    ...
    const descricao = [turma.cursoTurma, turma.periodoTurma]
        .filter(Boolean)
        .join(" · ");
    ...
    return {
        ...
        onClick: () => window.location.href = `disciplina.html` + "?idTurma=" + turma.idTurma
    };
}
