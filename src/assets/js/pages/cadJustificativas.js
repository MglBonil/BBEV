...
function parseValor(valor) {
    ...
    const normalized = valor.replace(/\s+/g, "").replace(",", ".");
...
async function criarJustificativa(event) {
    ...
    try {
        const response = await fetch(`${API}/categoria`, {
            ...
            body: JSON.stringify(novaJustificativa)
        });

        const result = await response.json().catch(() => null);

        if (!response.ok) {
            throw new Error(result?.message || `Erro ${response.status} ao cadastrar justificativa.`);
        }
        ...
        alert(error.message || "Erro ao cadastrar justificativa.");
    }
}
    ...
    try {
        ...
        if (categoria.statusCat != null) {
            ...
            if (categoria.statusCat) {
                ...
                if (statusInativo) statusInativo.checked = true;
            }
        }
    } catch (error) {
        console.error(error);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (id) {
        ...
        form.addEventListener("submit", criarJustificativa);
    }
});
